...
class DenoiseNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers, n_cond, d_cond, use_bias=True):
        super(DenoiseNN, self).__init__()
        self.n_layers = n_layers
        self.n_cond = n_cond
        #self.d_cond = d_cond
        self.cond_mlp = nn.Sequential(
            nn.Linear(n_cond, d_cond, bias=use_bias),
            nn.ReLU(),
            nn.Linear(d_cond, d_cond, bias=use_bias),
        )
        ...
        mlp_layers = [nn.Linear(input_dim+d_cond, hidden_dim, bias=use_bias)] + [nn.Linear(hidden_dim+d_cond, hidden_dim, bias=use_bias) for i in range(n_layers-2)]
        mlp_layers.append(nn.Linear(hidden_dim, input_dim, bias=use_bias))
        self.mlp = nn.ModuleList(mlp_layers)

        bn_layers = [nn.BatchNorm1d(hidden_dim, affine=use_bias) for i in range(n_layers-1)]
