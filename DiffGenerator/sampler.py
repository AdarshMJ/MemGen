...
# Molecular evaluation imports removed - not needed for generic graph datasets
...
class Sampler_mol(object):
    def __init__(self, config):
        ...
        # Import molecular dependencies only when needed
        try:
            from evaluation.molsets import get_all_metrics
            from utils.mol_utils import gen_mol, mols_to_smiles, load_smiles, canonicalize_smiles, mols_to_nx
            self.get_all_metrics = get_all_metrics
            self.gen_mol = gen_mol
            self.mols_to_smiles = mols_to_smiles
            self.load_smiles = load_smiles
            self.canonicalize_smiles = canonicalize_smiles
            self.mols_to_nx = mols_to_nx
        except ImportError as e:
            raise ImportError(f"Molecular evaluation dependencies not available. "
                            f"Please install rdkit and fcd_torch for molecule generation tasks. Error: {e}")
    ...
    def sample(self):
        ...
        train_smiles, test_smiles = self.load_smiles(self.configt.data.data)
        train_smiles, test_smiles = self.canonicalize_smiles(train_smiles), self.canonicalize_smiles(test_smiles)
        ...
        for r in range(num_sampling_rounds):
            ...
            gen_mols, num_mols_wo_correction = self.gen_mol(x, adj, self.configt.data.data)
            num_mols += len(gen_mols)
            num_mols_valid += num_mols_wo_correction

            smiles = self.mols_to_smiles(gen_mols)
        ...
        scores = self.get_all_metrics(gen=gen_smiles, k=len(gen_smiles), device=self.device[0], n_jobs=8, 
                                    test=test_smiles, train=train_smiles)
        scores_nspdk = eval_graph_list(self.test_graph_list, self.mols_to_nx(gen_mols), methods=['nspdk'])['nspdk']
